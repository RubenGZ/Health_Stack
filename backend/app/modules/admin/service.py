...
class AdminService:
    ...
    async def get_timeseries(db: AsyncSession, days: int) -> list[dict]:
        return await AdminRepository.get_timeseries(db, max(1, min(days, 365)))

    @staticmethod
    async def get_timeseries_multi(db: AsyncSession, days: int) -> list[dict]:
        return await AdminRepository.get_timeseries_multi(db, max(1, min(days, 365)))
    ...
    async def get_plans_distribution(db: AsyncSession) -> list[dict]:
        return await AdminRepository.get_plans_distribution(db)

    @staticmethod
    async def get_db_health(db: AsyncSession) -> list[dict]:
        return await AdminRepository.get_db_health(db)

    @staticmethod
    async def get_system_health(db: AsyncSession) -> list[dict]:
        return await AdminRepository.get_system_health(db)
    ...
    async def get_table_data(db: AsyncSession, table_name: str, page: int, limit: int) -> list[dict]:
        if table_name not in ALLOWED_TABLES:
            raise HTTPException(
                ...
                detail=f"Tabla '{table_name}' no disponible.",
            )
    ...
    async def patch_user(
        db: AsyncSession, admin_user_id: str, target_id: str, body: PatchUserRequest
    ) -> object:
        ...
        if body.role is not None:
            if body.role not in ("user", "admin"):
                raise HTTPException(
                    ...
                    detail="Rol inválido. Valores: 'user', 'admin'.",
                )
            if body.role == "user" and user.role == "admin":
                ...
                if admin_count <= 1:
                    raise HTTPException(
                        ...
                        detail="No puedes degradar al único admin activo.",
                    )
            user.role = body.role
        ...
        if body.plan is not None:
            if body.plan not in ("free", "pro", "elite"):
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    detail="Plan inválido. Valores: 'free', 'pro', 'elite'.",
                )
            user.plan = body.plan
        ...
        logger.info(
            "Admin %s actualizó usuario %s: role=%s plan=%s is_active=%s",
            admin_user_id[:8], target_id[:8], body.role, body.plan, body.is_active,
        )
        return user
