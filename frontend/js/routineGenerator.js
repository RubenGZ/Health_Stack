...
const RoutineGenerator = (function () {
  ...
  function pickExercises(group, equipment, count, skipGroups, injuries) {
    ...
    const pool = (window.Exercises?.getForRoutine(group, equipment)?.length
      ? window.Exercises.getForRoutine(group, equipment)
      : window.Exercises?.getForRoutine(group, 'full_gym') ?? []
    ).map(e => ({ name: e.name, sfr: e.sfr ?? 'medium', primary: e.primary ?? false }));
